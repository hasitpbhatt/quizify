...
export async function chat(messages: ChatMessage[], opts: ChatOptions): Promise<ChatResponse> {
  ...
  for (const currentModel of modelsToTry) {
    ...
    try {
      ...
      if (res.status === 429) {
        if (currentModel === modelsToTry[modelsToTry.length - 1]) throw new RateLimitError();
        await sleep(800);
        continue;
      }
      if (res.status >= 500) {
      ...
      if (currentModel === modelsToTry[modelsToTry.length - 1]) throw new NetworkError(err instanceof Error ? err.message : 'Unknown fetch error');
      await sleep(800);
    }
  }
