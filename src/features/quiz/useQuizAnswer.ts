...
export function localGrade(quiz: QuizData, given: string | string[]): SubmitResult {
  switch (quiz.format) {
    ...
    case 'trueFalse': {
      const correct =
        typeof given === 'string' &&
        given.trim().toLowerCase() === quiz.correctAnswer.trim().toLowerCase();
      return {
    ...
    case 'ordering': {
      ...
      const correct = givenArr.every(
        (item, i) => item.trim().toLowerCase() === expected[i].trim().toLowerCase(),
      );
      return {
    ...
    async (given: string | string[]) => {
      ...
      try {
        ...
        if (quiz.format === 'shortAnswer' || quiz.format === 'freeText') {
          ...
          try {
            result = await executePromptTask(
              gradeTask,
              {
                persona: 'curious',
                signal: undefined,
