...
- **2026-07-23** — Added Playwright e2e test harness. No webServer — app served via `page.route()` from `dist/`. IDB seeded directly via `page.evaluate`. Added `scripts/ghjq.ps1` to wrap `gh pr checks` (avoids PS 5.1 pipe-quoting hell with `gh --jq`). Use `.\scripts\ghjq.ps1 69` to check PR status. See `tests/e2e/` for the test pattern.
