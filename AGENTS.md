...
- **2026-07-21** — Split toolbar per D1 (Don Norman). Moved notebook toggle from CanvasPage bottom action bar to top Toolbar so brand, session selector, and notebook view toggle live in the top tier while actions (New, Theme, Export, Add Note) stay in the bottom bar. Added `canvasPage` prop to Toolbar. Added `.notebookToggle` CSS class. See `src/features/toolbar/Toolbar.tsx`, `src/app/App.tsx`, `src/features/canvas/CanvasPage.tsx`.
- **2026-07-23** — Added Playwright e2e test harness. No webServer — app served via `page.route()` from `dist/`. IDB seeded directly via `page.evaluate`. Added `scripts/ghjq.ps1` helper to work around PowerShell pipe-vs-jq-pipe quoting issue: use `.\scripts\ghjq.ps1 "pr view 69" "statusCheckRollup" '.statusCheckRollup[] | .name + " " + .conclusion'` instead of raw `gh --jq '...|...'`. See `tests/e2e/` for the test pattern.
